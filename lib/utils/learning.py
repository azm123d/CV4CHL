...
def load_pretrained_weights(model, checkpoint):
    ...
    for k, v in state_dict.items():
        ...
        if k == 'temp_embed':
            # maxlen设置为120，因此加载权重的时候只加载前120帧
            target_len = model_dict[k].size(1)
            if v.size(1) > target_len:
                v = v[:, :target_len, :, :]
...
def partial_train_layers(model, partial_list):
    ...
    print("INFO: Partial training: Only layers containing the following keywords will be trainable:", partial_list)
    
    for name, p in model.named_parameters():
        p.requires_grad = False
        for trainable in partial_list:
            if trainable in name:
                print(f"INFO: Partial training: {name} is trainable.")
...
def frozen_model(model):
    """Freeze all layers of a given model."""
    print("INFO: Freezing all layers of the pretrained model.")
    for name, p in model.named_parameters():
        p.requires_grad = False
    return model

def unfrozen_model(model):
    """Unfreeze all layers of a given model."""
    print("INFO: Unfreezing all layers of the pretrained model.")
    for name, p in model.named_parameters():
        p.requires_grad = True
    return model
...
def load_backbone(args):
    ...
    if args.backbone=='DSTformer':
        model_backbone = DSTformer(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep, 
                                   ...
                                   maxlen=args.maxlen, num_joints=args.num_DST_joints)
    ...
    elif args.backbone == 'Gait1':
        print("Using GaitNet_1 as backbone.")
        from lib.model.model_gait import GaitNet_1
        motionbert = DSTformer(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep, 
                                   depth=args.depth, num_heads=args.num_heads, mlp_ratio=args.mlp_ratio, norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                                   maxlen=args.maxlen, num_joints=args.num_DST_joints)
        model_backbone = GaitNet_1(
            backbone=motionbert, num_joints=args.num_joints, dim_rep=args.dim_rep,
...
def track_1_compute_acc(pred_logits, target):
    with torch.no_grad():
        pred_probs = torch.sigmoid(pred_logits)
        pred_labels = (pred_probs > 0.5).float()
        
        acc1 = (pred_labels == target).float().mean().item()
        p_i = pred_labels.view(pred_labels.size(0), -1).sum(dim=1)
        g_i = target.view(target.size(0), -1).sum(dim=1)
        
        # M is total number of samples (batch size in this case)
        rmse = torch.sqrt(((p_i - g_i) ** 2).mean()).item()
        nrmse = rmse / 34.0
        s1 = (acc1 + 1.0 - nrmse) / 2.0
        
    return s1


def track_2_compute_acc(pred_logits, target):
    with torch.no_grad():
        pred_probs = torch.softmax(pred_logits, dim=-1)  
        pred_labels = torch.argmax(pred_probs, dim=-1)  
        
        acc2 = (pred_labels == target).float().mean().item()
        f1_sum = 0.0
        num_classes = 5
        for k in range(num_classes):
            tp = ((pred_labels == k) & (target == k)).float().sum()
            pred_p = (pred_labels == k).float().sum()
            actual_p = (target == k).float().sum()
            
            precision = tp / (pred_p + 1e-8)
            recall = tp / (actual_p + 1e-8)
            
            if precision + recall > 0:
                f1_k = 2 * precision * recall / (precision + recall)
            else:
                f1_k = torch.tensor(0.0, device=target.device)
                
            f1_sum += f1_k.item()
            
        f1_macro = f1_sum / num_classes
        s2 = (acc2 + f1_macro) / 2.0
